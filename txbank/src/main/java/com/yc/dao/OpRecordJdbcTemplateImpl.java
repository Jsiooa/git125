...
public class OpRecordJdbcTemplateImpl implements OpRecordDao {
    ...
    public void insertOpRecord(OpRecord opRecord) {
        String sql="insert into oprecord(accountid,opmoney,optime,optype,transferid) value(?,?,now(),?,?)";
        this.jdbcTemplate.update(sql,opRecord.getAcccountid(),opRecord.getOpmoney(),
    ...
    public List<OpRecord> findOpRecord(int accountid) {
        List<OpRecord> list=this.jdbcTemplate.query(
                ...
                (resultSet,rowNum)->{
                    ...
                    opRecord.setAcccountid(resultSet.getInt(2));
                    ...
                    if (optype.equalsIgnoreCase("withdraw")){
                        ...
                        opRecord.setOpType(OpType.TEANSFER);
                    }
